...
public class SimpleServiceClient {
  ...
  SimpleServiceClient(String resourceBaseUrl) {
    LOGGER.trace("Build SimpleServiceClient with URL '{}'", resourceBaseUrl);
  ...
  public <C> C getResource(Class<C> responseType) {
    ...
    LOGGER.warn("Obtaining resource from resource URI {}.", uriBuilder.build(false).toUriString());
    ResponseEntity<C> response = restTemplate.exchange(uriBuilder.build(false).toUriString(), HttpMethod.GET, new HttpEntity<>(headers), responseType);
...
//  }
  ...
  public <C> C putResource(C resource, Class<C> responseType) {
    return putResource(resource, responseType, false);
  }
   ...
   * @return Puted resource.
   */
  public <C> C putResource(C resource, Class<C> responseType, boolean fetchEtag) {
    LOGGER.warn("Calling updateResource(#DataResource).");

    String destinationUri = buildUri();
    UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromHttpUrl(destinationUri).queryParams(queryParams);
      ResponseEntity<C> response;
    if (fetchEtag) {
      LOGGER.warn("Obtaining resource from resource URI {}.", uriBuilder.toUriString());
      response = restTemplate.exchange(uriBuilder.toUriString(), HttpMethod.GET, new HttpEntity<>(headers), responseType);
